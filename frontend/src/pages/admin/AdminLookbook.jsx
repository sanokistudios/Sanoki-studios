...
const AdminLookbook = () => {
  ...
  const loadLookbookImages = async () => {
    try {
      ...
      setLookbookImages(response.data.images || []);
    } catch (error) {
      console.error('Erreur lors du chargement des images lookbook:', error);
      toast.error('Erreur lors du chargement du lookbook');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenModal = (image = null) => {
    if (image) {
      ...
      const maxOrder = lookbookImages.length > 0 
        ? Math.max(...lookbookImages.map(img => img.order)) 
        : -1;
      setFormData({
        imageUrl: '',
        title: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingImage) {
        await lookbookAPI.update(editingImage._id, formData);
        toast.success('Image lookbook mise à jour');
      } else {
        await lookbookAPI.create(formData);
        toast.success('Image lookbook créée');
      }
      handleCloseModal();
  ...
  const moveImage = async (id, direction) => {
    const imageIndex = lookbookImages.findIndex(img => img._id === id);
    if (imageIndex === -1) return;

    const newIndex = direction === 'up' ? imageIndex - 1 : imageIndex + 1;
    if (newIndex < 0 || newIndex >= lookbookImages.length) return;

    // Échanger les ordres
    const updatedImages = [...lookbookImages];
    ...
    try {
      ...
      await lookbookAPI.updateOrder(imagesToUpdate);
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold">Gestion du Lookbook</h2>
        <button
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        {lookbookImages.length === 0 ? (
          <div className="p-12 text-center text-gray-500">
            Aucune photo dans le lookbook. Cliquez sur "Ajouter une photo" pour commencer.
          </div>
        ) : (
                  ...
                  <tr key={image._id} className="border-b hover:bg-gray-50">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <button
                            ...
                            className={`p-1 rounded ${index === lookbookImages.length - 1 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-gray-200'}`}
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm">
                        ...
                        {image.description && <p className="text-gray-600 text-xs mt-1">{image.description}</p>}
                      </div>
                    </td>
                    <td className="px-6 py-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  L'ordre détermine l'ordre d'affichage dans le lookbook (0 = première image)
                </p>
              </div>
